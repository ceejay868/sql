...
-- validate that the new vendor is inserted
...
WITH april_spend AS (
    SELECT 
        customer_id,
        SUM(cp.quantity * cp.cost_to_customer_per_qty) AS total_spent_in_april_2022
    FROM 
        customer_purchases AS cp
    WHERE 
